...
exports.getBookingofCurrentUser = async (req, res, next) => {
  try {
    ...
    if (!token || token == "null") {
      return res
        .status(401)
        .json({ success: false, message: "Login to see your booking" });
    }
...
exports.createBooking = async (req, res, next) => {
  ...
  if (tokenUser.role == "user" && tokenUser.id !== req.body.user) {
    return res
      ...
      .json({
        success: false,
        msg: "User token does not match the request. You can only book your own booking.",
      });
  }
  ...
  if (!dentist) {
    return res
      .status(400)
      .json({ success: false, msg: "Unknown dentist provided." });
  }
  ...
  if (bookingowner.booking) {
    return res
      .status(400)
      .json({ success: false, msg: "User had already booked." });
  }
...
exports.createBookingforcurrentuser = async (req, res, next) => {
  try {
    ...
    if (!token || token == "null") {
      return res
        .status(401)
        .json({ success: false, message: "Login to create your booking" });
    }
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
  ...
  try {
    ...
    if (tokenuser.role == "user" && decoded.id !== booking.user.id.toString()) {
      return res
        ...
        .json({
          success: false,
          msg: "User token does not match the request. You can only edit your own booking.",
        });
    }
    //if req.body.user is provided, it must be the same.
    if (
      req.body.user !== undefined &&
      req.body.user !== booking.user.toString()
    ) {
      return res
        .status(400)
        .json({
          success: false,
          msg: "Invalid update. Cannot change booking owner.",
        });
    }
...
exports.updateMyBooking = async (req, res, next) => {
  try {
    ...
    if (!tokenuser.booking || tokenuser.booking === null) {
      return res
        .status(400)
        .json({ success: false, message: "User haven't booked one." });
    }
    //update dentist and booking date
...
exports.deleteBooking = async (req, res, next) => {
  try {
    ...
    if (tokenuser.role === "user" && booking.user.toString() !== decoded.id) {
      return res
        .status(400)
        .json({ success: false, msg: "You can only delete your own booking." });
    }
...
exports.deletemyBooking = async (req, res, next) => {
  try {
    ...
    if (!tokenuser.booking || tokenuser.booking == "null") {
      return res
        .status(400)
        .json({ success: false, msg: "User has no booking" });
    }
    //delete booking
